...
async function pollTokenBuys(mint: string, priority = false): Promise<void> {
  ...
  try {
    ...
    if (!mintCheckpointed.has(mint)) {
      ...
      // Hard cap at 8 transactions for ALL discoveries (historical and real-time).
      // Consensus only needs 2 wallets scoring ≥80 — scanning 8 txns is more than
      // enough to find them. The old cap of 50 (historical) / unlimited (real-time)
      // queued up to 100 GMGN API calls per graduation, which was the direct cause
      // of recurring RATE_LIMIT_BANNED bans (100 calls × 4s = 400s of sustained
      // traffic, well over any reasonable per-minute limit).
      const earlyBuysCapped = earlyBuys.slice(0, 8);
